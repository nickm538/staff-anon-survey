...
export default function App() {
  ...
  async function load() {
    try {
      const [sr, fr] = await Promise.all([
        window.storage.list(PFXKEY, true),
        window.storage.list(FBKEY,  true),
      ]);
      if (sr?.keys?.length) {
        const rows = await Promise.all(sr.keys.map(k => window.storage.get(k, true)));
      ...
      if (fr?.keys?.length) {
        const frows = await Promise.all(fr.keys.map(k => window.storage.get(k, true)));
  ...
  async function submit() {
    ...
    try {
      await window.storage.set(`${PFXKEY}${ts}_${rnd}`, JSON.stringify(entry), true);
      setSubs(p => [...p, entry]);
    } catch {}
    if (feedback.trim()) {
      const fbEntry = { timestamp: ts, date, text: feedback.trim() };
      try {
        await window.storage.set(`${FBKEY}${ts}_${rnd}`, JSON.stringify(fbEntry), true);
